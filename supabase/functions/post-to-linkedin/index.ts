...
Deno.serve(async (req: Request) => {
  ...
  // Body size guard — 10 MB for multipart (images), 1 MB for JSON
  const isMultipart = (req.headers.get('content-type') ?? '').includes('multipart/form-data')
  const maxBodySize = isMultipart ? 10_485_760 : 1_048_576
  ...
  try {
    ...
    // Parse body — multipart (with optional image file) or JSON
    let org_urn: string, text: string, job_id: string | undefined
    let uploadedImageBytes: Uint8Array | null = null
    ...
    if (isMultipart) {
      ...
      try {
        ...
        return new Response(JSON.stringify({ error: 'invalid_form_data' }), { status: 400, headers: corsHeaders })
      }
      org_urn = (formData.get('org_urn') as string) ?? ''
      ...
      if (imageFile && imageFile.size > 0) {
        if (!['image/jpeg', 'image/png', 'image/webp', 'image/gif'].includes(imageFile.type)) {
          return new Response(JSON.stringify({ error: 'unsupported_image_type' }), { status: 400, headers: corsHeaders })
        }
        uploadedImageBytes = new Uint8Array(await imageFile.arrayBuffer())
        uploadedImageType = imageFile.type
      }
    } else {
    ...
    // Upload image to LinkedIn if provided (direct bytes take priority over job_id)
    let imageUrn: string | null = null

    if (uploadedImageBytes) {
      // Direct file upload path (from compose dialog file picker)
      ...
      const uploadRes = await fetch(uploadUrl, {
        method: 'PUT',
        headers: { 'Content-Type': uploadedImageType },
